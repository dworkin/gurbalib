...
void setup(void) {
   ...
   set_long("You are on the Border Walk, the long and narrow street " +
      ...
      "lies to your east. Immediately to the north the road reaches a " +
      "corner. To the south the walk heads toward the eastern town gate.");

   set_exits(([
      "north" : DIR + "/rooms/border_northeast",
      "south" : DIR + "/rooms/border_east7",
   ]));
}
