...
void do_give_obj(object obj) {
   if (obj->is_undroppable()) {
      this_player()->targetted_action("$N $vare unable to give away $o.", nil,
	 obj);
      return;
   }
...
void do_give_obj_str_liv(object obj, string s, object liv) {
   if (obj->is_undroppable()) {
      this_player()->targetted_action("$N $vare unable to give $o to $T.", liv,
	 obj);
      return;
   }
   ...
   if (obj->move(liv)) {
      ...
      this_player()->targetted_action("$N $vtry to give $o to $T, but $vfail.",
	 liv, obj);
   }
}
