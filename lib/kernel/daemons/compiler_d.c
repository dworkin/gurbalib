...
mapping query_inh_list() { return inh_list; }
mapping query_dep_list() { return dep_list; }
#endif
...
void clear_inherits(string file, int issue) {
  ...
  if(inh_list[progname]) {
    ...
    for(c=0;c<sizeof(inh);c++) {
      if(dep_list[inh[c]]) dep_list[inh[c]] -= ({progname});
    }
    ...
    rlimits(40;-1) {
      for (fcount = 0; fcount < sizeof(files); fcount++) {
        if(sscanf(files[fcount],f) == 1) {
          return dep_list[files[fcount]] ? dep_list[files[fcount]][..] : ({});
        }
      }
    }
  }
...
static string filter_for_edges(string * nodes)
{
  ...
  node = pile[0];
  if( !stringp(node) || sizeof(({ node }) & nodes) > 0 ) return nil;
  tmp = get_nodes(node);
  if(tmp && sizeof(tmp) > 0) {
    pile += tmp;
  } else if(!test_inheritable(node) || !dep_list[node] || !sizeof(dep_list[node])) {
    return node;
  }
}
...
/*
 * Find all inheritables that have more then one instance
 * registered.
 */
...
/*
 * Which files are included by the file provided
 * as argument.
 */
...
mixed * find_all_depending_programs(string file, varargs string * skip) {
   ...
   */
  if(!list) return ({ ({}), find_depending_objects(file) });
