...
nomask static void do_menu( mixed header, mixed *menu, varargs mixed footer ) {
  ...
  argcheck( header && (stringp( header ) || arrayp( header ) ), 1, "string or fcall array" );
  argcheck( arrayp( menu ), 2, "array" );
  argcheck( !footer || (stringp( footer ) || arrayp( footer ) ), 2, "empty, string or fcall array" );
  ...
  for( item = 0, items = sizeof( menu ); item < items; item++ ) {
    ...
    if(!arrayp(menu[item][3])) {
      error( "Error in menu item "+item+", fcall array expected in element 3" );
    }
