...
object query_conn_obj();

void create() {
  user_name = "";
	conn_obj = previous_object();
}

void open() {
  if( SITEBAN_D->is_banned(query_ip_number(conn_obj)) ) {
      /* site is banned */
      write_file( "/logs/logins", ctime( time() ) + "\t" + query_ip_number(conn_obj) +
         "\t" + "Banned Site\n" );
...
void input_name( string str ) {
  if( str == "" ) {
    ...
    if( file_exists( "/data/players/" + str + ".o" ) ) {
      ...
      player->set_name( user_name );
      player->set_proper_name( capitalize( user_name ) );
      if( SITEBAN_D->is_newbanned(query_ip_number(conn_obj)) ) {
...
void input_new_passwd( string str ) {
  ...
  write_file( "/logs/new_players", ctime( time() ) + "\t" + query_ip_number(conn_obj) +
      "\t" + query_name() + "\n" );
